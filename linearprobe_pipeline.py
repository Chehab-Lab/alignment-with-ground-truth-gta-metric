...
def probe(encoder_name, dataset_name, batch_size= 64, n_epochs= 20,
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if validate:
            ...
            if early_stopping_based_on_validation:
                ...
                if (len(recent_accs) >= 6) and (max(recent_accs) - min(recent_accs) < 0.05):
                    ...
                    save_checkpoint(chkpt_filepath, classifier, optimizer, epoch + 1, history, hyperparams, early_stopped=True, feature_drop_rate=feature_drop_rate)
                    break
        else:
            val_loss = None
            val_acc = None
        ...
        save_checkpoint(chkpt_filepath, classifier, optimizer, epoch + 1, history, hyperparams, early_stopped=False, feature_drop_rate=feature_drop_rate)
